...
public partial class PoissonSimulatorForm : Form
{
    ...
    // Compute the approximated Negative Binomial distribution for a given lambda
    private List<int> NegativeBinomialDistribution(double lambda, int n, int m)
    {
        // Create an m-long array of 0s to compute the distribution
        List<int> counts = new List<int>();
    ...
    private void button1_Click(object sender, EventArgs e)
    {
        ...
        List<int> negBinomial = NegativeBinomialDistribution(lambda, n, m);

        // Convert them to double
        List<double> poissonD = poisson.Select(x => (double)x).ToList();
        List<double> negBinomialD = negBinomial.Select(x => (double)x).ToList();
        ...
        PlotDistribution(new Rectangle(width + border, textHeight, width, height), true, negBinomialD);
